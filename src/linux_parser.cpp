...
double LinuxParser::ActiveJiffies(int pid) {
  ...
  if (stream.is_open()) {
    std::getline(stream, line);
    std::istringstream linestream(line);
    linestream >> x >> x >> x >> x >> x >> x >> x >> x >> x >> x >> x >> x >>
        x >> utime >> stime >> cutime >> cstime >> x >> x >> x >> x >>
        starttime;
  }
  total_time = utime + stime;
  total_time = total_time + cutime + cstime;
  seconds = uptime - (starttime / Hertz);
  cpu_usage = 1 * ((total_time / Hertz) / seconds);
  return cpu_usage;
}
...
int LinuxParser::TotalProcesses() {
  ...
  std::ifstream stream(kProcDirectory + kStatFilename);
  if (stream.is_open()) {
    while (std::getline(stream, line) && label != "processes") {
      std::istringstream linestream(line);
      linestream >> label >> processes;
    }
  }
  return processes;
}
...
int LinuxParser::RunningProcesses() {
  ...
  std::ifstream stream(kProcDirectory + kStatFilename);
  if (stream.is_open()) {
    while (std::getline(stream, line) && label != "procs_running") {
      std::istringstream linestream(line);
...
string LinuxParser::Ram(int pid) {
  ...
  if (stream.is_open()) {
    while (std::getline(stream, line) && label != "Vmsize:") {
      std::istringstream linestream(line);
...
string LinuxParser::Uid(int pid) {
  ...
  std::ifstream stream(kProcDirectory + to_string(pid) + kStatusFilename);
  if (stream.is_open()) {
    while (std::getline(stream, line) && label != "uid:") {
      std::istringstream linestream(line);
...
string LinuxParser::User(int pid) {
  string line;
  string user, x, uid;
  string myuid = LinuxParser::Uid(pid);
